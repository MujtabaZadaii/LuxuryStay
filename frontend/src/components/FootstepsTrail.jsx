...
export default function FootstepsTrail() {
  ...
  useEffect(() => {
    ...
    const feetPositions = [];
    const stepsNumber = 9;
    const iconSize = 40; // slightly smaller to look more delicate and elegant
    const mouseRepel = 25;
    ...
    // 2. Create SVG Feet templates and append to container
    const createFeet = () => {
    ...
    // 4. Update foot properties helper
    ...
    // 6. Intro trail animation
    const introAnimation = () => {
      introAnimationIsPlaying = true;
      const mouseCoords = { x: -100, y: window.innerHeight * 0.8 };
      gsap.timeline({
        onUpdate: () => {
  ...
  return (
    <>
      {/* Invisible template SVG with defs */}
      <svg viewBox="0 0 0 0" style={{ position: 'absolute', width: 0, height: 0, overflow: 'hidden' }}>
        <defs>
          {/* SVG path styled in a soft coral pink fill */}
          <path 
            id="feet-shape"
            fill="var(--color-coral)"
            style={{ fill: 'var(--color-coral)', opacity: 0.7 }}
      ...
      {/* Main footsteps rendering overlay */}
      <svg 
        ref={svgRef} 
        style={{ 
          position: 'fixed', 
